...
TRANSCRIPT_FILE = os.path.join(BASE_DIR, "data", "transcriptions.txt")
...
def load_data(spectrogram_dir=OUTPUT_DIR, transcript_file=TRANSCRIPT_FILE,img_height=128, img_width=128):
    print(f"Chemin des spectrogrammes : {os.path.abspath(spectrogram_dir)}")
    print(f"Chemin des transcriptions : {os.path.abspath(transcript_file)}")
    ...
    if not os.path.exists(transcript_file):
        raise FileNotFoundError(f"Fichier de transcription introuvable: {transcript_file}")
    ...
    for line in lines[:3]:  # Affiche 3 premières lignes
        print(f"  {line}")
    ...
    # Dans prepare_data.py, remplacez la ligne 119 par :
    with open(transcript_file, "r", encoding="utf-8") as f:
        lines = f.readlines()
    ...
    for line in lines:
        ...
        print(f"Exemples de lignes : {lines[:3]}")  # Affiche 3 premières lignes
    print(f"Nombre d'images chargées : {len(images)}")
    print(f"Longueurs des labels : {label_lengths}")

    if not label_lengths:
        raise ValueError(
            "Aucune donnée valide trouvée. Vérifiez :\n"
            f"1. Que {transcript_file} contient des lignes au format 'fichier|texte'\n"
            f"2. Que les fichiers PNG existent dans {spectrogram_dir}\n"
            "3. Que les textes ne contiennent que des lettres et espaces"
        )    
    print("Label lengths:", label_lengths)
    if not label_lengths:
        raise ValueError("Aucun label trouvé. Vérifie le fichier de transcription et les images.")
